                ...
                {notifications.length === 0 ? (
                  ...
                  notifications.map((notif) => (
                    <button
                      ...
                      className={`
                        w-full px-4 py-3 text-left hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors border-b border-slate-50 dark:border-slate-700/50 last:border-0
                        ${!notif.isRead ? 'bg-primary-50/30 dark:bg-primary-900/10' : ''}
                      `}
                    >
                      <div className="flex justify-between items-start mb-1">
                        <p className={`text-sm font-medium ${notif.isRead ? 'text-slate-700 dark:text-slate-300' : 'text-primary-600 dark:text-primary-400'}`}>
                          {notif.title}
                        </p>
                        {!notif.isRead && <span className="w-1.5 h-1.5 bg-primary-500 rounded-full mt-1.5" />}
                      </div>
