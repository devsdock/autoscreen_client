...
const Overview = () => {
  ...
  const quotes = apiData?.recentQuotes || (isLoading ? [] : (apiData ? [] : storeQuotes));
  ...
  const getFilterData = () => {
    ...
    if (timeRange === 'all') {
      return { dateText: 'All History', label: 'All Time', startDate: null, endDate: null };
    }
  ...
  const handleCancelBooking = async (bookingId) => {
    if (window.confirm('Are you sure you want to cancel this booking?')) {
      try {
        const response = await bookingService.cancelBooking(bookingId, 'Cancelled from dashboard history');
        if (response.success) {
              ...
              <div className="flex items-start justify-between mb-4">
                <div>
                  ...
                  <p className="text-xl font-bold text-slate-800 dark:text-white mt-0.5">
                    № {latestBooking.reference ? `BK-${latestBooking.reference.replace(/^BK-?/i, '')}` : 'Pending'}
                  </p>
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-3 mb-6">
                <div className="flex items-center gap-2 px-3 py-2 bg-slate-50 dark:bg-slate-800 rounded-lg">
                  ...
                  <span className="text-sm font-medium text-slate-700 dark:text-slate-300">
                    {typeof latestBooking.service === 'object' && latestBooking.service ? latestBooking.service.name : (latestBooking.service || 'General Service')}
                  </span>
                </div>
                <div className="flex items-center gap-2 px-3 py-2 bg-slate-50 dark:bg-slate-800 rounded-lg">
                  <MapPin size={16} className="text-slate-500 dark:text-slate-400" />
                  <span className="text-sm font-medium text-slate-700 dark:text-slate-300">
                    {latestBooking.address || latestBooking.city || latestBooking.location?.city || latestBooking.serviceAddress?.city || 'Location not specified'}
                  </span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium text-slate-700 dark:text-slate-300">{formatCurrency(latestBooking.price?.total || 0)}</p>
                  </div>
                </div>
              </div>
            </div>
              ...
              {filteredBookings.slice(0, 5).map((booking, index) => (
                <tr key={booking.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
                  <td className="py-4 px-4">
                    <span className="text-sm font-medium text-slate-800 dark:text-white">{booking.reference}</span>
                  </td>
                  <td className="py-4 px-4">
                    <span className="text-sm text-slate-600 dark:text-slate-400">
                      {typeof booking.service === 'object' && booking.service ? booking.service.name : (booking.service || 'General Service')}
                    </span>
                  </td>
                  <td className="py-4 px-4">
                    <span className="text-sm text-slate-600 dark:text-slate-400">
