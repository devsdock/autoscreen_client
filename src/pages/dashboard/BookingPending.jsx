...
const BookingPending = () => {
  ...
  const handlePayment = async () => {
    ...
    try {
      await bookingService.processBookingPayment(bookingId, {
  ...
  const statusConfig = {
    searching: {
      ...
      description: 'Your booking request is being broadcasted to providers in your area.',
    ...
    accepted: {
      ...
      description: 'Great news! A provider has accepted your booking. Complete payment to confirm.',
    ...
    confirmed: {
      ...
      description: 'Your booking is confirmed. The provider is scheduled for your selected time.',
  ...
  return (
    <div className="max-w-2xl mx-auto pb-12">
      {/* Status Banner */}
      <div className={`rounded-2xl ${currentStatus.bgColor} border ${currentStatus.borderColor} p-6 mb-6 text-center shadow-sm`}>
        ...
        <div className="mt-4 inline-flex items-center gap-2 px-4 py-2 bg-white/50 dark:bg-slate-800/50 rounded-full">
          ...
          <span className="font-mono font-semibold text-slate-900 dark:text-white">{booking.bookingNumber}</span>
        </div>
      </div>
          ...
          {booking.statusHistory?.map((step, index) => (
            <div key={index} className="flex items-start gap-4">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${
                ...
                {index <= 1 ? <CheckCircle size={18} /> : <div className="w-2 h-2 bg-current rounded-full" />}
              </div>
              <div className="flex-1">
                <p className={`font-medium ${index <= 1 ? 'text-slate-900 dark:text-white' : 'text-slate-500 dark:text-slate-400'}`}>
                  {step.status}
                </p>
                {step.timestamp && (
      ...
      {/* Booking Details */}
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 p-6 mb-6 shadow-sm">
        <h3 className="font-semibold text-slate-900 dark:text-white mb-4">Service Details</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm text-slate-500 dark:text-slate-400">Service & Glass</p>
              <p className="font-medium text-slate-900 dark:text-white capitalize">{booking.serviceType}</p>
              <p className="text-sm text-slate-600 dark:text-slate-400">{booking.glassType?.replace('_', ' ')}</p>
            </div>
          </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-600 dark:text-slate-400">{booking.scheduledTimeSlot}</p>
            </div>
          </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm text-slate-500 dark:text-slate-400">Service Location</p>
              <p className="font-medium text-slate-900 dark:text-white">{booking.serviceAddress?.addressLine1}</p>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 p-6 shadow-sm">
        {isPending && (
          <div className="flex flex-col items-center text-center gap-4 py-4">
            ...
            <div>
              <h4 className="font-semibold text-slate-900 dark:text-white">Broadcasting Request</h4>
              <p className="text-sm text-slate-600 dark:text-slate-400">
                Multiple providers have been notified. We'll alert you as soon as someone accepts.
              </p>
            </div>
          ...
          <div className="space-y-4">
            <div className="p-4 bg-green-50 dark:bg-green-900/10 rounded-xl border border-green-100 dark:border-green-900/30 text-center">
              ...
              <p className="text-sm text-green-700 dark:text-green-400">
                Please complete the payment within the next few minutes to secure this slot.
              </p>
            </div>
            <div className="flex gap-3">
              <Button variant="outline" className="flex-1">Decline</Button>
              <Button className="flex-1" onClick={() => setIsPaymentModalOpen(true)}>Pay Now</Button>
            </div>
          </div>
        )}
        ...
        {isConfirmed && (
          <div className="flex flex-col items-center text-center gap-4 py-4">
            ...
            <div>
              <h4 className="font-semibold text-slate-900 dark:text-white">Booking Confirmed</h4>
              <p className="text-sm text-slate-600 dark:text-slate-400">
                All set! You can view the full details and tracker in your bookings list.
              </p>
            </div>
