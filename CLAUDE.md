...
- **Legacy-quote gate confirmed on both files**: `{quote.supplyType && (...)}` on `QuotesNew.jsx`, `{quote?.supplyType && (...)}` (whole column + divider) on `QuoteDetailPanel.jsx`. Quotes created before the Phase 1 form deploy have no `supplyType` field and render no badge at all. Paired with the backend's same-date schema change that made `supplyType` truly optional (default removed from `models/Quote.js`, force-collapse dropped from `createQuote` + `createGuestQuoteWithAccount`) so the field is only ever persisted when the customer explicitly picks one of the two enum values.
...
- **`src/utils/dataMappers.js` — `generateTimeline()` partial-aware**: Two-event timeline for partial bookings (deposit paid at acceptance + balance paid at service-done).
  - New derivations near the top of `generateTimeline`: `isPartial = !!booking.partialPayment?.isActive`, `depositPct/balancePct` from `partialPayment.{depositPercentage,balancePercentage}`, `depositAmt/balanceAmt` from top-level booking fields, `balancePaid = booking.balanceStatus === "paid"`, `formatZAR()` helper.
  - `_resolveBalanceMethodLabel()` helper resolves `balancePaymentMethod` + `paymentSubMethod` to a customer-friendly label: cash → "Cash", eft → "EFT", speed_point → "Speed Point", card+tokenized → "Card (Auto-charge)", card+payment_link → "Card (Pay Link)", card → "Card".
  - **Step 4 (acceptance)**: `Deposit Paid R{amount} ({pct}%)` when partial — overrides the existing cash/card-after/prepayment branches. Date prefers `actualTimes.confirmedAt` then `partialPayment.snapshotAt` then `createdAt`.
  - **Step 9.5 (NEW, between Service Done and Completed)**: `Balance Paid R{amount} ({pct}%) via {method}` when `isPartial && balancePaid`. Date is `booking.balancePaidAt`. Skipped entirely for non-partial bookings.
- **Zero-regression**: Non-partial bookings unchanged — all new logic gated behind `isPartial` flag.
- **Build verified** (`npm run build`).

### 1 May 2026 (Partial Payment — Architecture & Conventions)

This entry captures the architectural patterns that the timeline change above plugs into. Once admin enables partial payment, EVERY non-insurance accept flow (prepayment / cash / card-on-completion subModes) charges a deposit upfront via Paystack and tracks the balance separately.

- **`src/services/publicSettingsService.js` — `getPartialPaymentConfig()`**: New service-layer call to `GET /public/settings/partial-payment` returning `{ isActive, depositPercentage, balancePercentage }`. Falls back to `{ isActive: false, depositPercentage: 100, balancePercentage: 0 }` on error. Components fetch this on mount; "off" config means legacy "Pay in Full" everywhere.
- **`src/components/dashboard/PartialPaymentIntroModal.jsx` — NEW Step 1 explainer**: Pure presentation modal (no API calls, no commit). Renders the deposit/balance breakdown card (Pay Now %, After Service %, Total job value) before the customer picks a balance method. Continue → advances to PaymentMethodModal/PaymentModal; Cancel → aborts. Reused only from `QuoteDetailPanel.handleAcceptAndPay`.
- **Accept flow split (`QuoteDetailPanel.jsx`)**: `handleAcceptAndPay` → if partial active and not insurance, store `pendingAcceptResponse` and show intro modal. Continue calls `continueAcceptAfterIntro(response)` which contains the original method-picker / PaymentModal branching. Insurance flows skip partial entirely (separate accounting). The `pendingAcceptResponse` state is preserved across the intro → method-picker handoff.
- **`authorization_url` redirect convention extended to ALL accept paths**: Pre-partial, only `prepayment` and `card_after_tokenize.tokenized` returned a Paystack `authorization_url` from the backend. With partial active, **cash and card_link also return one** (the deposit charge). `QuoteDetailPanel` cash and card branches now uniformly check `if (res.authorization_url) window.location.href = res.authorization_url; return;` BEFORE running the success-toast/redirect-url paths. Tokenize without auth URL is still treated as an error.
- **`PaymentSuccess.jsx` — `needsVerify` widened**: Was `Boolean(reference && (mode === "prepayment" || mode === "card_tokenized"))`. Now `Boolean(reference)` — any return-from-Paystack with a reference triggers `verifyPaystack`. Required because partial-mode cash and card_link now also do a Paystack round-trip for the deposit. Verify endpoint is idempotent so racing webhook/verify is safe.
- **`PaymentMethodModal.jsx` — Prepayment hidden when partial active**: When `partialActive`, the standalone "Pay now by card" tile is omitted (Pay-via-Link already covers deposit-now-balance-later). A blue compact reminder bar at the top echoes `R{deposit} deposit charged now. Choose how to settle the R{balance} balance after service:`. Each remaining card (Cash, Card-on-Completion → Pay-link / Add-card-now) swaps its body copy to a partial-aware variant explaining the deposit charge + balance settlement method. `card_tokenized` sub-mode is shown but is technically Phase 2 (real-deposit + tokenize) — copy already in place.
- **`PaymentModal.jsx` — Partial-aware breakdown + surcharge math**: New `partialActive = config.isActive && !isInsuranceR0 && amount > 0` flag. Title becomes "Accept & Pay Deposit (X%)" / button becomes "Pay Deposit R{depositAmount}". Below the existing Total row a dashed divider exposes "Pay Now ({depositPct}% deposit)" + "Balance due at completion ({balancePct}%)". **Surcharge correctness**: `coversFees` surcharge is now calculated on `chargeAmount = partialActive ? depositAmount : amount` — only the actual Paystack charge bears the gateway-fee surcharge. R0 insurance bypasses partial entirely.
- **`isPartialPending` display gate (4 surfaces)**: Strict gate `partialPayment.isActive === true && balanceStatus === "pending" && Number(balanceAmount) > 0` is the SOLE trigger for "balance owed" UI. Used by `BookingCard.jsx`, `BookingDetailDrawer.jsx`, `QuoteDetailPanel.jsx` confirmed-provider bar, and `BookAppointment.jsx` `ProviderPaidBar`. Each surface:
  - Headline amount swaps from `paidAmount`/`total` → `balanceAmount` (what's still owed).
  - Sub-line shows `Deposit R{depositAmount} paid · Total R{total}`.
  - Badge becomes blue `Deposit Paid · Balance {method}` where method derives from `paymentOption + paymentSubMethod` (cash → "in cash", card+tokenized → "auto-charged", card+payment_link → "via link").
- **`BookingDetailDrawer.jsx` — Partial breakdown + cancel-quote awareness**: Price section gets a dashed-divider partial breakdown (deposit paid amount + balance amount with status-coloured pill: paid=emerald, waived=slate, pending=amber + helper text on whether to expect cash-at-tech vs link-by-email). **STRICT GATE** also requires `Number(depositAmount) > 0` so a half-populated legacy record can never accidentally render partial UI. The cancel-quote dialog reads `cancelQuote.partialDepositOnly` (backend-supplied) — when true, it shows "Deposit Paid" / "Balance (waived on cancel)" rows instead of "Booking Total", and the no-refund footer copy switches to "...equals your deposit".
- **`StatusBadge.jsx` — `deposit_paid` payment status**: New entry (blue 100/700, blue dot, label "Deposit Paid") for both raw enum (`deposit_paid`) and the human-formatted form (`"Deposit Paid"`). Mirrored in `dataMappers.js` `getNormalizedPaymentStatus()` which now returns `"Deposit Paid"` for `paymentStatus === "deposit_paid"`.
- **`Payments.jsx` — "Deposit Paid" treated as committed-but-pending**: `totalSpent` and `lastPaid` now include `status === "Deposit Paid"` (deposit money landed). The Pending tab count also includes "Deposit Paid" so customers see what they still owe surface up.
- **`BookingCard.jsx` — Mode flag**: Local `isDepositPaid = paymentStatus === "deposit_paid"` is folded into `isPaid` (booking is committed). When `hasBalanceDue` (partial + pending + balance>0), the standard "Paid" sub-label is suppressed and replaced with a blue `Deposit Paid · Balance R{x} due` pill (with title-attribute showing both amounts).
- **`dataMappers.js` — `mapBooking()` partial fields**: `partialPayment`, `depositAmount`, `balanceAmount`, `balanceStatus`, `balancePaymentMethod`, `balancePaidAt` now flow through to the mapped booking object so all card/drawer/panel surfaces have one canonical source.
- **`Insurance.jsx` — Currency locale fix**: `formatCurrency` switched from `en-ZA` (comma decimal) to `en-US` (period decimal) for portal-wide consistency (matches the same fix applied to `PaymentMethodModal` on 21 April).
...
- **Root cause — email deep-link opens over an empty list**: When a customer clicks an email link for a booking in one of those filtered-out states, `Bookings.jsx:193-227` falls through to `fetchBookingDetails(id)` and opens `BookingDetailDrawer` — but the list sections behind the drawer don't contain that booking (filtered out for the same reason). User perceives this as "details shown but list side not visible". Single root cause — same fix as above.
- **`src/pages/dashboard/Bookings.jsx:24`**: `upcoming` extended to `"confirmed,accepted,in-progress,searching,arrived,service-done,payment-pending"`. Arrived / service-done / payment-pending bookings now render in the Upcoming section and email deep-links find them in `allBookings` before the fallback fetch.
- **Root cause — "Scheduled" badge persists on completed bookings**: `BookingCard.jsx:84` had `isScheduled = isCommitted && hasSchedule`. A completed booking still has `paymentStatus: "paid"` (so `isCommitted` is true) and `scheduledDate` (so `hasSchedule` is true), so the ternary at line 193-204 painted "Scheduled" and never reached the `<StatusBadge>` that would render "Completed".
- **`src/components/dashboard/BookingCard.jsx:84`**: Tightened to `isScheduled = category === "upcoming" && isCommitted && hasSchedule`. Completed / cancelled bookings fall through to the `<StatusBadge>` branch and render the correct lifecycle label. Drawer at `BookingDetailDrawer.jsx:318-331` already had the right guard (`status === "confirmed"`) so drawer was unaffected.
- **Root cause — "Unpaid" badge misleads for cash / card-on-completion modes**: `BookingDetailDrawer.jsx:332-336` rendered `<StatusBadge status={booking.paymentStatus} type="payment" />` unconditionally. For FPO bookings the DB `paymentStatus` is legitimately `"unpaid"` until money lands, so the drawer painted a red "Unpaid" pill next to the correct "Cash on Completion" / "Pay After Service" label — mixed messaging. `BookingCard.jsx:330-363` already derives the right pill from `paymentOption` / `paymentSubMethod` / `cardAuth`; provider portal got the equivalent fix on 14 April 2026 (N-099, `JobDetail.jsx`). The drawer was the last holdout.
- **`src/components/dashboard/BookingDetailDrawer.jsx:332-379`**: Replaced the raw `StatusBadge` with an IIFE that derives the pill from `paymentOption`. Prepayment or settled states (`paid`, `insurance_direct`, `refunded`, `partially_refunded`) still use the standard `<StatusBadge>`. Cash mode → emerald "Cash on Completion". Card-on-completion with `status === "payment-pending"` → red "Payment Required". Card-on-completion with `cardAuth.last4` → violet "Card ••••{last4}". Card-on-completion with no card on file → amber "Pay After Service". All pills match the StatusBadge `md` size (`px-3 py-1 text-sm`, 6px colored dot, full dark-mode variants).
- **`src/components/dashboard/BookingDetailDrawer.jsx:380-393`** — Workshop/Mobile pill parity: the service-mode pill beside the status badges used a smaller custom shape (`px-2 py-0.5 rounded text-[11px]`, no dot, inline `style` colors) that didn't visually align with the other `size="md"` badges. Rewritten to use the same StatusBadge dot-pill shape: mobile → blue-100/700 with blue dot, workshop → amber-100/700 with amber dot. Matches "Scheduled" / "Cash on Completion" / "Pay After Service" visually.
- **Zero backend change**: Customer `bookingController.js` `getBookings` (line 375-384) and `getBooking` (line 430-445) have no `.select()` projection — all FPO fields (`paymentOption`, `paymentSubMethod`, `cashReceipt`, `paymentLink`, `cardAuth`) already come through untouched. Backend status lifecycle, `validTransitions`, and `Booking.js` enum untouched.
...
- **`PaymentMethodModal.jsx` — Two-step selection for the passive methods**: New `selectedMethod` state tracks a tentative pick. Clicking "Pay cash when done" or "Pay via link later" now only highlights the card (ring + "Selected" pill) and reveals a bottom-right Confirm button ("Confirm Cash Payment" / "Confirm Pay Via Link"). `onSelect(method)` only fires when the user clicks Confirm. Clicking the same card a second time toggles the selection off. Prepayment and "Add card now" keep their existing click-to-commit behavior (prepayment has PaymentModal's Pay button as its own confirm; "Add card now" is handled by the backend's deferred acceptance — Paystack card-save is the real commit).
- **Coordination with the deferred-tokenize backend change** (see `autoscreen_node` changelog, same date): "Add card now" now calls an endpoint that only initializes Paystack tokenize — no booking created, no responses rejected. If the customer backs out of Paystack, the quote stays in `quoted` state with all providers selectable. No frontend change needed in `QuoteDetailPanel` for the new flow (the old `isCardSetupPending` detection still works for legacy orphaned bookings from before the backend fix).
- **Visual design**: Selected cards get a soft 2px ring in the method's accent color (emerald for cash, amber-dark for payment_link) plus a coloured "Selected" pill next to the headline. The Confirm button sits next to the existing Cancel in the footer, using the method's gradient (emerald for cash, amber for payment_link) to keep colour continuity.

### 16 April 2026 (Quote Detail — Mode-Aware Payment Badge & Notice Copy)

- **`QuoteDetailPanel.jsx` — Payment badge in provider bar is now mode-aware**: The flat "Paid in full" pill was misleading for cash bookings, Path B card-after bookings (pay via link after service), and Path A tokenized bookings (card on file, charged later). Rewrote the badge block as a self-contained IIFE that branches on `booking.paymentOption` + `paymentSubMethod` + `cardAuth.last4`:
  - Insurance (registered provider): "Insurance Covered" / "Excess Paid" (unchanged).
  - `paymentOption === "cash"`: emerald pill "Cash on Completion".
  - `paymentOption === "card_on_completion" + subMode === "payment_link"`: amber pill "Pay After Service" (matches `BookingCard.jsx` wording).
  - `paymentOption === "card_on_completion" + subMode === "tokenized" + cardAuth.last4`: violet pill "Card on File ••XXXX" (matches `BookingCard.jsx` + provider JobDetail patterns).
  - Default (prepayment): green "Paid in full".
- **`QuoteDetailPanel.jsx` — Notice bar text per mode**: Replaced the flat "Payment successful!" copy (shown when `bookingIsConfirmed && !scheduledDate`) with a per-mode IIFE. Cash → "Booking confirmed! You'll pay cash on completion…". payment_link → "Booking confirmed! A payment link will be sent after service…". tokenized (with card) → "Booking confirmed! Your card will be charged automatically after service…". Prepayment → "Payment successful!".
- **Zero backend changes**: All fields (`paymentOption`, `paymentSubMethod`, `cardAuth`) are already populated on the quote's booking via the populate extension from the same date (see the card-after tokenize changelog entry below). Other portals (`BookingCard.jsx`, `BookingDetailDrawer.jsx`) already had equivalent mode-aware rendering — this just brings the quote detail view into parity.

### 16 April 2026 (Card-After Tokenize — Abandoned-Flow Recovery)

- **Root cause**: For the "Pay after service → Add card now" flow (Path A / `subMode: "tokenized"`), the backend `processQuoteCardAfterAcceptance` (`autoscreen_node/utils/processPaystackCharge.js:1527`) creates a `status: "confirmed"` booking BEFORE the Paystack R1 tokenize redirect. If the customer opens Paystack and presses Back without entering card details, the booking stays `confirmed` with `cardAuth.authorizationCode === null`. The client UI then showed the full "Paid in full" provider bar, journey step "Paid" as complete, and "Book Your Appointment" CTA — misleading because no card was saved and no money will ever be collected without manual admin action.
- **`QuoteDetailPanel.jsx` — `isCardSetupPending` flag**: New derived flag `!!booking && booking.paymentOption === "card_on_completion" && booking.paymentSubMethod === "tokenized" && !booking.cardAuth?.authorizationCode`. `bookingIsConfirmed` now additionally requires `!isCardSetupPending`, so "Paid in full" bar, "Book Your Appointment" button, and the confirmed-appointment card are all suppressed during this state. Journey step 3 (Paid) fires on `(bookingNeedsPayment || isCardSetupPending)` so the stepper shows the right current step.
- **`QuoteDetailPanel.jsx` — Banner + notice copy**: Notice bar and Payment Pending banner both get a dedicated `isCardSetupPending` branch — "Card Setup Not Completed — Your card wasn't saved on Paystack. Complete card setup to confirm your booking." Banner button becomes "Complete Card Setup" (loading-guarded) and calls the new `paymentService.retryCardTokenize(bookingId)`.
- **`paymentService.js` — `retryCardTokenize(bookingId)` added**: POSTs to `/customer/payments/retry-card-tokenize/:bookingId`.
- **Backend coordination** (see `autoscreen_node` changelog, same date): New `POST /customer/payments/retry-card-tokenize/:bookingId` endpoint regenerates a Paystack tokenize URL with a fresh reference. Customer-ownership enforced via `Booking.findOne({ _id, customer })`. 60-second rapid-retry guard (derived from existing reference timestamp) prevents double R1 holds from rapid clicks / dual tabs. `getQuotes` and `getQuote` populate select extended with `paymentOption paymentSubMethod cardAuth` so the client receives the data needed for the flag.

### 16 April 2026 (PaymentModal Loading Feedback + Quote "Accepting" State Unlock)

- **Root cause — Auto-accepted on back**: Backend `initializeQuotePayment` sets `quote.status = "accepting"` + `quote.acceptedResponse = response._id` BEFORE the Paystack redirect. `dataMappers.js` maps `"accepting"` → `"Accepted"`. If the customer clicked Pay, reached Paystack, and pressed Back without filling card details, returning to the quote showed the provider as "Accepted" and all other provider cards as "Rejected/disabled" — visually locking them into a provider they never actually paid for.
- **`QuoteDetailPanel.jsx` — `isPaymentInitiatedNotComplete` flag**: New derived flag `quote.rawStatus === "accepting" && !bookingIsConfirmed`. `renderCard` now only treats a response as accepted when `matchesAcceptedId && bookingIsConfirmed` (payment actually succeeded). `thisRejected` only applies when `bookingIsConfirmed`. Card `disabled` prop changed from `isAccepted || isClosed` to `bookingIsConfirmed || isClosed`. During pre-payment "accepting" state, all provider cards stay selectable so customers can retry payment OR pick a different provider (backend already overwrites `acceptedResponse` on new `initializeQuotePayment` call with a different `responseId`).
- **`QuoteDetailPanel.jsx` — Notice + Payment Pending banner copy**: New messaging when `isPaymentInitiatedNotComplete` is true — "Payment Not Completed. Your last payment wasn't completed. Retry below, or pick a different provider." Distinguishes from the generic "Payment required" banner shown for bookings that never started payment.
- **Root cause — Modal feedback gap**: Paystack init can take 10-40s (subaccount creation, VAT fetch, transaction init API call). During that wait, `PaymentModal` only showed a tiny spinner inside the Pay button, and backdrop/Escape/X-button/Cancel were all still usable, so customers closed the modal and saw no UI for 30+ seconds before the browser navigated to Paystack — many re-clicked "Accept & Pay" thinking it was broken.
- **`PaymentModal.jsx` — Close guards during loading**: `handleClose` now returns early if `loading === true`. `handleEscape` also checks loading. Backdrop `onClick={handleClose}` inherits the guard. `loading` added to the Escape effect's dependency array.
- **`PaymentModal.jsx` — Full-content loading overlay**: Absolute-positioned overlay (`z-10`, blurred white/slate-900 background) renders over the modal body when `loading` is true. Large spinner + "Redirecting to secure payment…" headline + "This can take up to 30 seconds. Please don't close this window or press back." subline + "Secured by Paystack" footer with lock icon. Replaces the near-invisible button-only spinner with clear, on-brand feedback.
- **Zero backend changes**: Both fixes are client-only, respecting zero-regression. The backend "accepting" + pre-set `acceptedResponse` pattern still works for retries (Paystack metadata carries `responseId`; `processQuoteChargeSuccess` reads `quote.acceptedResponse`). The client just stops treating pre-payment state as a final commitment visually.
